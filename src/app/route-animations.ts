import {
  trigger,
  transition,
  style,
  query,
  group,
  animateChild,
  animate,
  keyframes,
} from '@angular/animations';

export const slider = trigger('routeAnimations', [
  transition('inicio => contacto', slideTo('right')),
  transition('inicio => nosotros', slideTo('right')),
  transition('inicio => *', slideTo('right')),

  transition('contacto => inicio', slideTo('left')),
  transition('contacto => nosotros', slideTo('right')),
  transition('contacto => *', slideTo('right')),

  transition('nosotros => inicio', slideTo('left')),
  transition('nosotros => contacto', slideTo('left')),
  transition('nosotros => *', slideTo('right')),
]);

function slideTo(direction: string) {
  const optional = { optional: true };
  return [
    query(
      ':enter, :leave',
      [
        style({
          position: 'absolute',
          top: 0,
          [direction]: 0,
          width: '100%',
        }),
      ],
      optional
    ),
    query(':enter', [style({ [direction]: '-100%' })]),
    group([
      query(
        ':leave',
        [animate('600ms ease', style({ [direction]: '100%' }))],
        optional
      ),
      query(':enter', [animate('600ms ease', style({ [direction]: '0%' }))]),
    ]),
    // Normalize the page style... Might not be necessary

    // Required only if you have child animations on the page
    // query(':leave', animateChild()),
    // query(':enter', animateChild()),
  ];
}
